...
for text in train_df['text'].dropna().tolist():
    tokens = preprocess_text(text)
    word_counter.update(tokens)

vocab_size = len(word_counter) + 2  # Garante que o vocabulário seja consistente
most_common_words = word_counter.most_common(vocab_size - 2)
word2idx = {"<pad>": 0, "<unk>": 1}
word2idx.update({word: idx + 2 for idx, (word, _) in enumerate(most_common_words)})
...
# Modelo
model = CustomModelTextClassifier(vocab_size=len(word2idx)).to(device)
...
# Criar DataLoader
batch_size = 16
train_dataset = torch_dataset(train_df['text'].dropna().tolist(), train_df['is_hate_speech'].tolist(), vocab=word2idx)
...
for epoch in range(epochs):
    ...
    # Ajustar learning rate com o scheduler **antes do print**
